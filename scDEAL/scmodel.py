...
################################################# START SECTION OF SINGLE CELL DATA REPROCESSING #################################################
    ...
    if args.foundation_model == 'gexpr':
        adata = sc.read_h5ad(f'../data/gexpr/singlecell/{data_name}_HVG.h5ad')
        data = adata.X
    elif args.foundation_model == 'scRobust':
        data = np.load(f'../data/embeddings/scRobust/sc_{data_name}_HVG2000.npy')
    elif args.foundation_model == 'scGPT':
        data = np.load(f'../data/embeddings/scGPT/raw_{data_name}_HVG.npy')
    elif args.foundation_model == 'scFoundation':
        data = np.load(f'../data/embeddings/scFoundation/sc_{data_name}_50M-0.1B-res_singlecell_cell_embedding_t4_resolution.npy')
...
################################################# START SECTION OF LOADING BULK DATA  #################################################
    ...
    if args.foundation_model == 'gexpr':
        if args.data_type =='Bulk':
            data_r = pd.read_csv(f'../data/gexpr/Bulk/Bulk_{args.drug}_HVG.csv',index_col=0)
        elif args.data_type == 'scStyleBulk':
            data_r = pd.read_csv(f'../data/gexpr/scStyleBulk/scStyleBulk_{args.drug}_HVG.csv',index_col=0)
    
    elif args.foundation_model =='scRobust':
        data_r = np.load(f'../data/embeddings/scRobust/Bulk_{args.drug}_HVG2000.npy')
        
    elif args.foundation_model =='scGPT':
        data_r = np.load(f'../data/embeddings/scGPT/Bulk_{args.drug}_HVG.npy')
        
    elif args.foundation_model =='scFoundation':
        if args.data_type =='Bulk':
            data_r = np.load(f'../data/embeddings/scFoundation/Bulk_{args.drug}_50M-0.1B-res_bulk_cell_embedding_f1_resolution.npy')
        elif args.data_type == 'scStyleBulk':
            data_r = np.load(f'../data/embeddings/scFoundation/scStyleBulk_{args.drug}_50M-0.1B-res_singlecell_cell_embedding_t4_resolution.npy')
    
    label_r = scanpy.read_h5ad(f'../data/gexpr/Bulk/Bulk_{args.drug}_HVG.h5ad').obs['sensitive']
...
################################################# START SECTION OF TRANSFER LEARNING TRAINING #################################################
    ...
    ################################################# END SECTION OF ANALYSIS FOR scRNA-Seq DATA #################################################
        ...
        f1score = f1_score(Y_test, lb_results)
        
        print('F1 : ', round(f1score, 4))
        print('AUC : ', round(auroc_score,4))
        print('AP : ', round(ap_score, 4))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--foundation_model', type=str, default='gexpr', help='gexpr/scRobust/scGPT/scFoundation')
    parser.add_argument('--data_type', type=str, default='Bulk', help='Bulk or scStyleBulk')
